...
read -p "Enter the IP address you will listen on: " tun0
# "$(ip addr show | grep eth0 |grep -o 'inet [0-9]*\.[0-9]*\.[0-9]*\.[0-9]*' | grep -o '[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*')"
...
for i in commands_*.txt; do 
  cat $i | sed "s/\$tun0/$tun0/g" | sed "s/\$port/$port/g" | sed "s/\$http_port/$http_port/g" | sed "s/\$smb_share/$smb_share/g" | sed "s/\$smb_share/$smb_share/g" | pygmentize; 
done
